...
const InteractivePage: React.FC = () => {
  ...
  const [regPoints, setRegPoints] = useState<Point[]>([])
  ...
  const drawGraph = (ctx: CanvasRenderingContext2D) => {
    ...
    points.forEach((point, index) => {
      ...
      ctx.beginPath();
      const radius = index === hoveredPoint || index === draggingPoint ? 8 : 6; // Enlarge on hover/drag
    ...
    <div className="flex flex-row items-center p-4">
      ...
      <div> 
        <canvas
          ...
          width={WIDTH}
          height={HEIGHT}
          className="border border-gray-300 w-full mx-auto overflow-x-auto"
          style={{ cursor: hoveredPoint !== null ? "pointer" : "default" }} // Change cursor on hover
